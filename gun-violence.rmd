...
We find that we have an extremely high number of incidents with exactly one person and logarithmically less incidents 
with more than one person. We also observe two interesting situations: There seems to be a single incident with 100 
people involved, as well as about 25k incidents with no people involved at all.

When we investigate the issue with the most participants, we find it to be the [Orlando nightclub shooting](https://en.wikipedia.org/wiki/Orlando_nightclub_shooting) at December 12th, 2016, where a 29-year old security 
guard killed 49 nightclub visitors in a terroristic attack, injured 50 more and eventually got killed by the police.
...
The following map shows the locations of incidents. Each incident is marked with 1% transparency, i.e. it takes (at least) one hundred
incidents in the very same location to render the marker fully red. As we can see on the map, locations of high incident counts seem 
to cluster around the western coast of the USA and fill out the entire eastern half of the USA; at the same time, Hawaii also
pops up in red, with two visible peaks indicating at least 200 incidents.
...
The effect of having a high incident counts in eastern USA in contrast to a comparatively lower incident counts in the west
might be explained by population density in these regions. Information about this is currently not available to me, so I'll leave
that open for further research.

We can now also identify incidents where either a high number of people involed were killed (red marker) or injured (yellow).
...
When we investigate the relations the number of people killed and injured for each incident, we don't find much of 
a meaningful relationship in general. It appears that for high numbers of people involved, the numbers are positively
correlated. For small numbers of killings, the number of injured people also tends to be small (and vice versa).

```{r, killed-injured, echo=FALSE}
ggplot() +
  geom_jitter(data = inc, aes(x = n_killed, y = n_injured), alpha = .2)
```
When we zoom into the bottom-left corner of the previous plot, we find a triangular relationship between the number of killings and
the number of injured people. This is backed up by the plot for people involved in an incident; we already know that it is very
likely for an incident to involve less than ten people, and very unlikely for an incident to involve more than ten. The triangular shape
in this plot occurs since in this dataset, the number of people involved in an incident is the sum of people killed and injured in it. 
Interestingly, the solid triangular shape suggests that the probability distribution of being injured or killed is actually uniform; 
that is it would be equally likely to bekilled or injured when being involved in an incident. 
However, this is a global observation and does not take into account the type of crime. For a gang fight, for example, 
it could be assumed that every participant is actively engaging in the crime - if that type of incident would be
the majority, the result (being killed or surviving injured) would be governed by chance.
